...
![Version: 8.0.0](https://img.shields.io/badge/Version-8.0.0-informational?style=flat-square) ![AppVersion: v1.30.0](https://img.shields.io/badge/AppVersion-v1.30.0-informational?style=flat-square)
...
|-----|------|---------|-------------|
| backup | object | `{"backupServerName":"","externalSecret":{"enabled":true},"method":"objectStore","objectStore":{"data":{"compression":"snappy","encryption":"","jobs":1},"destinationBucket":"postgres-backups","destinationPathOverride":"","endpointCA":{"create":false,"key":"","name":""},"endpointCredentialsOverride":"","endpointURL":"http://garage-main.garage:3900","externalSecretCredentialPath":"/garage/home-infra/postgres-backups","isWALArchiver":true,"retentionPolicy":"30d","wal":{"compression":"snappy","encryption":"","maxParallel":1}},"scheduledBackups":[]}` | Backup settings |
...
| backup.objectStore.data.jobs | int | `1` | Number of data files to be archived or restored in parallel. |
...
| backup.objectStore.endpointCA | object | `{"create":false,"key":"","name":""}` | Specifies a CA bundle to validate a privately signed certificate. |
| backup.objectStore.endpointCA.create | bool | `false` | Creates a secret with the given value if true, otherwise uses an existing secret. |
| backup.objectStore.endpointCredentialsOverride | string | `""` | Override secret name that contains S3 credentials, should contain the keys ACCESS_KEY_ID and ACCESS_SECRET_KEY |
| backup.objectStore.endpointURL | string | `"http://garage-main.garage:3900"` | Overrides the provider specific default endpoint. |
| backup.objectStore.externalSecretCredentialPath | string | `"/garage/home-infra/postgres-backups"` | Path points at path in cluster store that contains the keys ACCESS_KEY_ID and ACCESS_SECRET_KEY |
...
| backup.objectStore.wal | object | `{"compression":"snappy","encryption":"","maxParallel":1}` | Storage |
...
| backup.objectStore.wal.maxParallel | int | `1` | Number of WAL files to be archived or restored in parallel. |
